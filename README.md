...
- **무기 (14종)**: 일섬(정면 즉발 섬광), 마력탄(자동 추적), 투척 단검, 투척 도끼(포물선), 성역(신성 장판·밀침), 수호 영혼(회전 궤도), 선풍검(주위 360° 베기), 장창(관통), 룬 마탄(강력·관통 마법탄), 독무(광역 둔화 장판), **연쇄 뇌격**(무작위 낙뢰가 근처 적으로 연쇄), **부메랑**(왕복하며 두 번 타격), **화염 지뢰**(설치형 폭발 + 불타는 장판), **흡혈 사슬**(피해의 일부를 체력으로 흡수). 레벨별 강화.
- **패시브 아이템 (17종)**: 최대 체력 · 위력 · 이동속도 · 쿨다운 · 범위 · 투사체 수/속도 · 자력 · 방어 · 체력 재생 · 치명타율/피해 · 회피 · 부활 · 골드 · 행운 · 경험치.
- **캐릭터 (5종)**: 기사(탱커) / 마법사(위력·범위) / 도적(속도·치명타) / 사제(오라·재생) / 전사(궤도·탱키). 각자 시작 무기가 다름.
- **적 & 웨이브**: 시간이 지날수록 적의 종류·수·체력·피해가 증가. 3분 이후에는 일반 적 일부가 **챔피언**으로 승격 — 금빛으로 빛나고 체력·크기가 크게 높으며 처치 시 대형 보석을 떨굽니다(물량이 아닌 스파이크로 긴장감을 줍니다). 별도로 주기적인 **정예**(보물상자 드롭)와 **보스**(Demon King · The Eternal Night)가 등장. 화면 밖에 보물상자가 있으면 화면 가장자리에 금색 나침반 화살표가 방향을 알려줍니다.
- **돌발 이벤트**: 런 중간에 정해진 타이밍으로 **골드 러시**(골드 2배 + 동전 비), **핏빛 달**(적 쇄도 + 경험치 1.5배), **유령 행렬**(골드를 잘 떨어뜨리는 망령 무리 습격)이 발생해 8분의 리듬을 바꿉니다.
- **하드 모드 (저주 계약)**: 첫 승리 후 메뉴에서 해금. 단계당 적 체력 +35%·피해 +15%·이동속도 +4%·동시 출현 +12%, 대신 골드 +25%·경험치 +15%. 해당 단계에서 승리해야 다음 단계가 열립니다 (최대 7단계).
